...
const BackgroundControls = ({ project }) => {
  ...
  const handleBackgroundRemoval = async () => {
    ...
    if (!mainImage) return;
    if (!project) {
      toast.error("Project is not loaded yet. Please try again.");
      return;
    }
    ...
    try {
      ...
      if (!currentImageUrl) {
        toast.error("No image found in the project.");
        return;
      }
  ...
  const searchUnsplashImages = async () => {
    ...
    try {
      const response = await fetch(
        `${UNSPLASH_API_URL}/search/photos?query=${encodeURIComponent(searchQuery)}&per_page=50`,
        {
          headers: {
  ...
  return (
    <div className='space-y-6 '>
      ...
      <Tabs defaultValue="color" className="w-full">
        <TabsList className="grid w-full grid-cols-2 p-1 rounded-xl bg-white/5 backdrop-blur-lg border border-white/10 gap-2">
          <TabsTrigger
            value="color"
            className="flex items-center justify-center data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:via-purple-600 data-[state=active]:to-cyan-500 data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-blue-500/25 data-[state=active]:scale-105 text-white/70 hover:text-white transition-all duration-200 rounded-lg"
          >
